...
TaxSearch::TaxSearch(SeqToKMers seqSplitter, int hits_max, bool best_only,
    ...
    nodes(200000, NameNode("NA", -1, -2, 0)) {   // Size of RDP is around 200000
...
void TaxSearch::readKMerIndex(std::string &file_path) {
    ...
    while (std::getline(input, line)) {
        ...
        for (auto node_it = nodeTokens.begin(); node_it != nodeTokens.end();
             ++node_it) {
...
void TaxSearch::fill_node_tax_row(int node_id, vector<string> &node_tax_row) {
    ...
    do {
        ...
        for (auto word_it = words.rbegin(); word_it != words.rend();
             ++word_it) {
...
std::vector<int> TaxSearch::searchNodes(std::string &sequence) {
    ...
    for (auto level_it = level_vector.rbegin(); level_it != level_vector.rend();
        level_it++) {
        // Reset histogram.
...
void TaxSearch::pickBestHits(std::vector<int> &ret, int kmer_size) {
    ...
    for (auto node_it = node_counts.begin(); node_it != node_counts.end();
         ++node_it) {
...
std::ostream & operator<<(std::ostream & os, Hit hit) {
    os << "Hit[" << std::get<0>(hit) << ", " << std::get<1>(hit) << ", " <<
       std::get<2>(hit) << "]";
