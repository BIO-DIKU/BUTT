...
#ifndef BUTT_SEARCH_TAXSEARCH_H_
#define BUTT_SEARCH_TAXSEARCH_H_
...
class TaxSearchException: public std::exception {
 public:
    ...
    TaxSearchException(const TaxSearchException &e):
...
class TaxSearch {
 public:
     ...
     * That is if a hit consists of 344 kmers out of 345 possible, only hits
     * with 344 kmers are included.
     ...
     * @param tax_index_path File path for the taxonomy index file fewer kmers
     * than the total amount of kmers x coverage it will be filtered.
     */
    TaxSearch(SeqToKMers seqSplitter, int hits_max, bool best_only,
              double coverage, TaxConsensus* consensus_builder,
              std::string &kmer_index_path, std::string &tax_index_path);
     ...
     * Example:
     * ("Query 1","K#Bacteria(100);P#Proteobacteria(100); \
     *  C#Gammaproteobacteria(80);O#Vibrionales(75);F#;G#;S#")
     */
    ...
    /**
     * @brief Method to search for a given sequence. First the sequence is
     * broken down into unique k-mers using the seqSplitter.  For each
     * taxonomic level starting from the deepest, all nodes containing these
     * k-mers are looked up. The nodes containing at least a ratio of
     * TaxSearch::coverage of the k-mers are considered node-hits. If there are
     * no node-hits at a taxonomic level, we move to the next level.
     *
 ...
 private:
    ...
    // Working memory for building histograms in search. Arrays are used in the
    // hope that memset will be faster than vector::assign.
    std::vector<unsigned int> node_counts;

    /**
     * @brief Reads the taxonomy index files into data structure and prepare
     * for the search. If the file does not exist, a FileNotFoundException will
     * be raised.
     * @param file_path
     */
    ...
    /**
     * @brief Reads the kmer index files into data structure and prepare for
     * the search. If the file does not exist, a FileNotFoundException will be
     * raised.
     * @param file_path
     */
...
#endif   // BUTT_SEARCH_TAXSEARCH_H_
